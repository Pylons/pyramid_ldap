...
class _LDAPQuery(object):
    ...
    ``%(...)s`` placeholders in ``filter_tmpl`` or ``base_dn``.
    ...
    def execute(self, conn, **kw):
        if not self.filter_tmpl:
            ...
            logger.debug('result generated by string formatting: %r' % result)
            return result
        else:
            cache_key = (
...
def ldap_set_login_query(config, base_dn, filter_tmpl='',
    ...
    building syntetic search results, with just a DN and no any attributes.
